function [coe,output]  = func_copy_MAIN_Planetary_Approach_RevB(v_inf_Entry_I,zp,var,Primary,input)
% Choose between:
...
%
% OUTPUTS
% output.RA_Entry_a           =  RA_Entry_a; 
% output.DEC_Entry_a          = DEC_Entry_a;
% output.r_vec_SOI_Entry_a_I  = r_vec_SOI_Entry_a_I;
% output.i_range              = [i_min ,i_max];
% output.argp_range           = [w(1)  ,w(2) ];
% output.argp_range2          = [w(3)  ,w(4) ];
...
i = var;
    ...
    for k=1:K
        if i(k)<i_min || i(k)>i_max 
            ...
            fprintf('For i = %.1f deg, no possible trajectory exists. \n',i(k))
            fprintf('Choosing min viable orbital inclination instead: i = %.1f\n',i_min)
            i(k) = i_min;
...
if strcmp(User_Input,'ARGP')
    ...
    for k=1:K
        if (ARGP(k)>w(2) && ARGP(k)<w(3)) || (ARGP(k)>w(4) && ARGP(k)<w(1))
            fprintf('For ARGP = %.1f deg, no possible trajectory exists. \n',ARGP(k))
            fprintf('Choosing viable value instead: ARGP = %.1f deg \n',w(1) )
            ARGP(k) = w(1);
            % return
        else
...
for k=1:K
    ...
    if      any(imag(R_PaI(:,:,k)) ~= 0);
        warning('Transformation matrix has imaginary components.');
    elseif  any(imag(R_PbI(:,:,k)) ~= 0);
        warning('Transformation matrix has imaginary components.');
    end
