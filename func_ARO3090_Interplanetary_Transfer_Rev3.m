function [v2_vec,JD_Dep, JD_Arr] = func_ARO3090_Interplanetary_Transfer_Rev3(Planet_1,Planet_2,LaunchDate,Additional_Days,TOF, UIaxishand)
    % Planet_1   : Departure
    % Plante_2   : Arrival
    % Launch Date: datetime object.
    % UIaxishand : UI axis handle. 
    % v2_vec   : Velocity, Arrival Planet     
    warning('off'); % HZ.
    ...
    %% Launch Date 
    yyyy = year(LaunchDate); 
    mm = month(LaunchDate); 
    dd = day(LaunchDate); % Convert from datetime to double array.HZ
    % yyyy = 2024;  mm = 10;  dd = 18;
    ...
    fprintf('Launch Date(YYYY-MM-DD): %.0f- %.0f-%.0f \n',yyyy,mm,dd)
    tic
    ...
    z0_S =-100;
    while func_Fun(z0_S,TOF*86400,mu,r1,r2,A_S) < 0
        z0_S = z0_S + 0.1;
    end
    z0_L = -100;
    while func_Fun(z0_L,TOF*86400,mu,r1,r2,A_L) < 0
        z0_L = z0_L + 0.1;
    end
    
    z_S = fsolve(@(z_S)func_Fun(z_S,TOF*86400,mu,r1,r2,A_S),z0_S,Options);
    z_L = fsolve(@(z_L)func_Fun(z_L,TOF*86400,mu,r1,r2,A_L),z0_L,Options);
    
    y_S = r1 + r2 + A_S.*(z_S.*func_stumpS(z_S) - 1)./sqrt(func_stumpC(z_S));
    y_L = r1 + r2 + A_L.*(z_L.*func_stumpS(z_L) - 1)./sqrt(func_stumpC(z_L));
    ...
    v_inf_a = vinf_arr_vec; % Output Variable.HZ
...
for j=1:size(TAa,2)
            r_PFa(:,j) = h^2/mu/(1+e*cosd(TAa(j)))*[cosd(TAa(j)),sind(TAa(j)),0]'; %11/11/2024 Error here
                                                                                   % Moved Sub fx's to individual .m files
            r_Ia(:,j)  = R_IP*r_PFa(:,j);
        end
        TAb = TA1:0.01:360;
    ...
    %% Plots
    % updating to direct plots to specific axis handle.
    % hold(UIaxishand, 'on'); 
    % set(UIaxishand,'units','normalized','position',[0,0.06,0.5,0.83]) 
    % fig = ancestor(UIaxishand, 'figure');
    % fig.Color               =[0,0,0];
    % fig.Name                ="Interplanetary Transfer";
    % fig.Units        ="normalized";
    % fig.Position     =[0,0.06,0.5,0.83];
    UIaxishand.GridColor    =[0,0,0];
    UIaxishand.Color        =[0,0,0];
    UIaxishand.NextPlot     = "add";
    try
        view(UIaxishand, [Om,25])
    catch
        view(UIaxishand, [real(Om),25])
        fprintf('Warning: Om is reporting complex value of %f', Om)
    end
    ...
    tlt_t = title(UIaxishand, [strt1,strt2]);
        tlt_t.Color = 'w'; 
        tlt_t.FontSize = 13; 
    
    l_S = plot3(UIaxishand, 0,0,0,'markeredgecolor',[1,1,0.4],'markerfacecolor',[1,1,0.4],'marker','o','markersize',10,'color','none');
    if TA1<TA2
        l = plot3(UIaxishand, r_I(1,:)/AU,r_I(2,:)/AU,r_I(3,:)/AU,'color',color_traj,'linewidth',1.5);
    else
        l = plot3(UIaxishand, r_Ia(1,:)/AU,r_Ia(2,:)/AU,r_Ia(3,:)/AU,'color',color_traj,'linewidth',1.5);
            plot3(UIaxishand, r_Ib(1,:)/AU,r_Ib(2,:)/AU,r_Ib(3,:)/AU,'color',color_traj,'linewidth',1.5);
    end
    plot3(UIaxishand, r_I_P1(1,:)/AU,r_I_P1(2,:)/AU,r_I_P1(3,:)/AU,'color',color_P1,'linewidth',0.5);
    plot3(UIaxishand, r_I_P2(1,:)/AU,r_I_P2(2,:)/AU,r_I_P2(3,:)/AU,'color',color_P2,'linewidth',0.5);
    
    l_P1     = plot3(UIaxishand, r1_vec(1,1)/AU,r1_vec(2,1)/AU,r1_vec(3,1)/AU,'markeredgecolor',color_P1,'markerfacecolor',color_P1,'marker','o','markersize',8,'color',color_P1);
    l_P2     = plot3(UIaxishand, r2_vec(1,1)/AU,r2_vec(2,1)/AU,r2_vec(3,1)/AU,'markeredgecolor',color_P2,'markerfacecolor',color_P2,'marker','o','markersize',8,'color',color_P2);
    text_P1 = sprintf('   %s on %d-%d-%d',Planet_1,yyyyD,mmD,ddD);
    text_P2 = sprintf('   %s on %d-%d-%d',Planet_2,yyyyA,mmA,ddA);
    text(UIaxishand, r1_vec(1,1)/AU+.1,r1_vec(2,1)/AU+.1,r1_vec(3,1)/AU+.1,text_P1,'Color','w')%,'FontSize',16,'interpreter','latex');
    text(UIaxishand, r2_vec(1,1)/AU+.1,r2_vec(2,1)/AU+.1,r2_vec(3,1)/AU+.1,text_P2,'Color','w')%,'FontSize',16,'interpreter','latex');
        
    axis(UIaxishand,'equal')
    
    leg = legend(UIaxishand, [l_S,l_P1,l_P2,l],'Sun',sprintf('%s',Planet_1),sprintf('%s',Planet_2),'Interplanetary Transfer Orbit');
        leg.TextColor=[1 1 1];
